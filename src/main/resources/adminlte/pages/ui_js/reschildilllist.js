...
$(function () {
                    ...
                    success: function (data) {
                        if (data.data.data == null) {
                            ...
                            for(var i = 0;i<data.data.data.length;i++){
                                ...
                                data.data.data[i].checkdate =  timetrans(data.data.data[i].checkdate).replace('T'," ");
                                data.data.data[i].action = "<a href='javascript:void(0);'onclick='modifyIll("+ JSON.stringify(data.data.data[i]) + ")'  class='down btn btn-default btn-xs'><i class='fa fa-arrow-down'></i> 录入检测数据</a>";
                            }
    ...
    $("#a_childmodify").click(function () {
        var id = $('#illid').html();
        //修改录入信息
        var clicktype = "modifychildill";
        var checkdate = $("#reginput_checkdate").val()+":00";
        var sex = $("#regmodalselect_sex").find("option:selected").text();
        var bcheckres = $("#regmodalselect_bcheckres").find("option:selected").text();
        var checkperson = $("#reginput_checkperson").val();
        if(checkdate == null || bcheckres == null || checkperson == null
            || checkdate == "" || bcheckres == "" || checkperson == ""){
            alert("信息填写不完整！");
            return;
        }
        ...
        inputsenddata.bcheckres = bcheckres;
                    ...
                    success: function (data) {
                        if (data.data.data == null) {
                            ...
                            for(var i = 0;i<data.data.data.length;i++){
                                ...
                                data.data.data[i].checkperson = data.data.data[i].checkperson;
                                data.data.data[i].checkdate =  timetrans(data.data.data[i].checkdate).replace('T'," ");
                                data.data.data[i].action = "<a href='javascript:void(0);'onclick='modifyIll("+ JSON.stringify(data.data.data[i]) + ")'  class='down btn btn-default btn-xs'><i class='fa fa-arrow-down'></i> 录入检测结果</a>";
                            }
                            viewdata = $.extend(true,[],data.data.data);
                            dt.fnAddData(viewdata);
                            dt.fnPageChange(page);//加载跳转
...
function modifyIll(obj) {
    ...
    $("#regmodalselect_bcheckres").val(obj.bcheckres);
