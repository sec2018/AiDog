...
public interface NeckletMapper {
    ...
    @Select("select a.nec_id as mid, b.voltage,b.temperature,b.updatetime from (select distinct nec_id from dogdev.dog where districtcode like concat(#{districtCode},'%') and nec_id != '-1' order by nec_id) a left join (SELECT t.mid,t.voltage,t.temperature,t.updatetime FROM (SELECT * FROM dogdev.sys_laytime order by updatetime desc) t  GROUP BY t.mid) b on a.nec_id = b.mid")
    List<SysLaytime> selectViewLayTime(String districtCode);
    @Select("select a.nec_id  as mid,b.status,b.uimodifyflag,b.hardmodifyflag from (select distinct nec_id from dogdev.dog where districtcode like concat(#{districtCode},'%') and nec_id != '-1' order by nec_id) a inner join dogdev.sys_deviceconf b where a.nec_id = b.mid")
